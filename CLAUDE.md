...
**Current state (2026-06-12):** Live paper-trading log analyzed (snapshot runs 2026-03-18 → 2026-06-05). The "v2.4 STABLE segment" was NOT healthy: the engine kept scanning but strategies stopped emitting signals — May had 507 scans yet only 15 signals (April had 317), June only 1. Root cause of the live `ml_direct` collapse (April 213 → May 14 → June 0 signals) is NOT model staleness: `ml_direct` has **no directional edge** and was deliberately RETIRED 2026-06-02 (direction AUC ≈ 0.51; see `docs/research/ml_direct_retirement_2026_06_02.md`) — its apparent live "+10.8k @75% WR" is luck (fat-tail avgLoss −2655). Its intended frequency replacement MACD-M15 ALSO failed validation (git `ab26b0b`, all 4 gates). The live machine was running a stale/pruned April lineup on fumes. **There is currently NO validated deployable positive-edge strategy** — only macd_crossover H1 has a thin edge (PF ~1.10, ADX-gated ~1.36). DO NOT revive ml_direct (high re-enable bar; retraining on the same features is forbidden). The 375 "risk rejections" are 81% legitimate filters (H4-trend 79, session 63); the 62 "setup bug" rejections were March-only and already fixed. The 2026-04-28 go/no-go is long past with no decision recorded. **The project's real center of gravity is now the RESEARCH track** (Guardian falsification engine + Phase 12b trend, which DSR-passes the corrected gates). See memory `project-paper-trading-analysis`. (v4 Phase 10 crypto-momentum OOS was RED.)
